...
# Import datasets
d <- read.csv("data/Complied.CleanedTM.Meta4.csv")
...
```{r pred_fig_mod9_with_raw, echo = F, fig.width = 9, fig.height=5, cache=FALSE}
...
```{r composite_fig_1, echo = F, cache = F, fig.width=9, fig.height=8}

study_level_fig <- study_level_fig +
  theme(plot.margin = margin(t = 20, r = 5, b = 5, l = 5))

sample_level_fig <- sample_level_fig +
  theme(plot.margin = margin(t = 20, r = 5, b = 5, l = 5))

plot_grid(study_level_fig,
          sample_level_fig,
          labels = c("Site Detections", "Sample detections"),
          ncol = 1,
          rel_heights = c(.85, 1.25))
...
plot_model(trad_mod1,
  ...
  labs(x = "Organism", y = "Predicted Probabilities for Active vs. Passive Traditional Sampling",
       color = NULL, title = "") 

```

# Log odds for study level analysis  
```{r meta_analysis_data_prep, echo = F, eval = F}

# tried this but not worth it. Converting the GLMM estimates to log odds avoids introducing arbitrary continuity corrections.
# 
# d1 <- d1 %>% 
#   mutate(raw_prop = detections / n,
#          prop = pmax(pmin(raw_prop, 1 - 1e-6), 1e-6),
#          logit_props = log(prop / (1 - prop)),
#          var_logit_props = 1 / (n * prop * (1 - prop)))
# 
# hist(pmin(d1$prop, 1 - 1e-6))
# hist(d1$raw_prop)
# d2 <- d2 %>% 
#   mutate(prop = detections / samples,
#          prop = pmax(pmin(prop, 1 - 1e-6), 1e-6),
#          logit_props = log(prop / (1 - prop)),
#          var_logit_props = 1 / (samples * prop * (1 - prop))) 

```

```{r meta_analysis_study_level, echo = F, eval = F}

# ignore. This chunk is not run.
study_meta1 <- rma.mv(yi = logit_props,
                      V = var_logit_props,
                      mods = ~ detect_method,
                      random = list(~ 1 | paper,
                                    ~ detect_method | study),
...
```{r log_odds_study_level, echo = F}

# get overall contrast:
study_odds_overall <- emmeans(study_mod2, ~ detect_method,
                              type = "link")

study_contr_overall <- contrast(study_odds_overall,
                                method = "pairwise")

study_overall <- summary(study_contr_overall, infer = TRUE) %>% 
  as.data.frame() %>% 
  mutate(organism = "Overall")


# Get organism level contrasts:
study_odds <- emmeans(study_mod2, ~ detect_method | organism,
                type = "link")

contr_study <- contrast(study_odds, method = "pairwise")




summary(contr_study, infer = TRUE) %>% 
  as.data.frame() %>% 
  bind_rows(., study_overall) %>% 
  mutate(star = case_when(
    p.value <= 0.05 ~ "*",
    TRUE ~ " "),
    organism = factor(organism, levels = c(
      "Arthropod", "Reptile", "Mammal",
      "Crustacean", "Amphibian", "Fish",
      "Mollusc", "Overall"
    ))) %>% 
  ggplot(aes(x = organism, y = estimate)) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL),
                width = 0) +
  geom_text(aes(x = organism, y = asymp.UCL,
                label = star),
            hjust = -1) +
  coord_flip() +
  theme_minimal(base_size = 18) +
...
```{r log_odds_sample_level, echo = F}

# get overall contrast:
sample_odds_overall <- emmeans(mod9, ~ detect_method,
                              type = "link")

sample_contr_overall <- contrast(sample_odds_overall,
                                method = "pairwise")

sample_overall <- summary(sample_contr_overall, infer = TRUE) %>% 
  as.data.frame() %>% 
  mutate(organism = "Overall")


# Get organism level contrasts:
sample_odds <- emmeans(mod9, ~ detect_method | organism,
                type = "link")

contr_sample <- contrast(sample_odds, method = "pairwise")




summary(contr_sample, infer = TRUE) %>% 
  as.data.frame() %>% 
  bind_rows(., sample_overall) %>% 
  mutate(star = case_when(
    p.value <= 0.05 ~ "*",
    TRUE ~ " "),
    organism = factor(organism, levels = c(
...
```{r log_odds_figure, echo = F, fig.width=6, fig.height=9}

paper_odds_data <- summary(contr_study, infer = TRUE) %>% 
  as.data.frame() %>% 
  bind_rows(., study_overall) %>% 
  mutate(star = case_when(
    p.value <= 0.05 ~ "*",
...
sample_odds_data <- summary(contr_sample, infer = TRUE) %>% 
  as.data.frame() %>% 
  bind_rows(., sample_overall) %>% 
  mutate(star = case_when(
    p.value <= 0.05 ~ "*",
...
```{r results_table, echo = F}

paper_level_contrasts <- summary(contr_study, infer = TRUE) %>% 
  as.data.frame() %>% 
  bind_rows(., study_overall) %>% 
  mutate(res = paste(organism, "paper", sep = "-")) %>% 
  column_to_rownames("res") %>%
  select(-contrast, -SE, -df, -organism) 


sample_level_contrasts <- summary(contr_sample, infer = TRUE) %>% 
  as.data.frame() %>% 
  bind_rows(., sample_overall) %>% 
  mutate(res = paste(organism, "sample", sep = "_")) %>% 
  column_to_rownames("res") %>%
  select(-contrast, -SE, -df, -organism) 


# bind_rows(paper_level_contrasts, sample_level_contrasts) %>% 
#   t() %>% as.data.frame() %>% 
#   select(sort(names(.))) %>% 
#   round(., 2) %>% 
#   write.csv(, file = "data/contrast_results.csv")
